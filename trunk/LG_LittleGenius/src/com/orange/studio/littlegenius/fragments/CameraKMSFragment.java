...
public class CameraKMSFragment extends BaseFragment implements OnClickListener,
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		if (mView == null) {
			mView = inflater.inflate(R.layout.fragment_camera_layout,
					container, false);
			initView();
			initListener();
		} else {
	...
	private void loadVideoKMS() {
		...
		if (mLoadVideoKMSTask == null
				|| mLoadVideoKMSTask.getStatus() == Status.FINISHED) {
			mLoadVideoKMSTask = new LoadVideoKMSTask();
			mLoadVideoKMSTask.execute();
		}
	}

	private class LoadVideoKMSTask extends
			AsyncTask<Void, Void, List<VideoKMSDTO>> {
		@Override
		...
		protected List<VideoKMSDTO> doInBackground(Void... params) {
			return CommonModel.getInstance().getListVideoFromServer(
					URLRequest.VIDEO_KMS_URL);
		}
		...
		protected void onPostExecute(List<VideoKMSDTO> result) {
			...
			try {
				...
				// mList.add(new VideoKMSDTO(id, "Video " + id,
	...
	public void onItemClick(AdapterView<?> arg0, View arg1, int position,
		...
		try {
			...
			// Toast.makeText(getActivity(), item.name,
