...
public class HomeFragment extends BaseFragment {
	...
	class LoadDataTask extends AsyncTask<String, Void, String> {
		...
		protected void onPostExecute(String valus) {
			...
			try {
				if (content != null) {
					// String str_content
					// ="<b><font color=\"#0181D8\" size=\"35px\">Class Schedule : </font></b><br><ul>";
					// for (int i = 0; i < content.size(); i++) {
					// str_content +=
					// "<li><font color=\"#0181D8\" size=\"30px\">" +
					// content.get(i) + "</font></li><br>";
					// }
					// str_content +=
					...
					for (int i = (content.size() / 2); i < content.size(); i++) {
						str_content2 += "<font color=\"#0181D8\" size=\"30px\">"
								+ content.get(i) + "</font>";
					}
					str_content2 += "";
	...
	public void onClick(View gId) {
		...
		case R.id.programBtn:
			getActivity().getIntent().putExtra(LG_CommonUtils.PROGRAM_PARAM,
	...
	public void initView() {
		...
		mTestimonialBtn = (LinearLayout) mView
				.findViewById(R.id.testimonialsBtn);
				...
				if (result != null && result.result == 1) {
					...
					if (jArr != null && jArr.length() > 0) {
						...
						for (int i = 0; i < jArr.length(); i++) {
							mList.add(new HomeSliderDTO(String.valueOf(i), jArr
									.getString(i)));
						}
	...
	private void loadData() {
		if (mLoadDataTask == null
				|| mLoadDataTask.getStatus() == Status.FINISHED) {
			String url_select = LG_CommonUtils.URL_SCHEDULE;
			mLoadDataTask = new LoadDataTask();
			mLoadDataTask.execute(url_select);
		}
		if (mLoadSlideBannerTask == null
				|| mLoadSlideBannerTask.getStatus() == Status.FINISHED) {
